...
app.get("/filter", (req, res) => {
  ...
  const filteredJokes = jokes.filter((joke) => type === joke.jokeType);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const deleteIdx = jokes.findIndex((joke) => joke.id === id);

  if (deleteIdx > -1) {
    jokes.splice(deleteIdx, 1);
    res.status(200).json({ message: `joke is deleted successfully!` });
  } else {
    res.status(404).json({ error: `joke with id: ${id} is not found!` });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (key === masterKey) {
    ...
    res
      .status(404)
      .json({ error: `You don't have authorization to delete the jokes!` });
  }
});
